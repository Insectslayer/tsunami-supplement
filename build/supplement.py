...
def prepare_quantitative_markdown(markdown: str) -> str:
    """Adapt package-oriented README contents for the reviewer-facing web page."""
    analysis_item = (
        "- `Tsunami_quantitative_analysis.ipynb` — complete quantitative "
        "analysis with stored outputs."
    )
    requirements_item = (
        "- `requirements.txt` — Python dependencies used to reproduce the notebook."
    )
    markdown = markdown.replace(
        "This package contains the analysis-ready quantitative data and the executable analysis notebook for the TSUNAMI VR locomotion study. ",
        "",
        1,
    )
    markdown = markdown.replace(
    ...
    reproducibility_heading = "## Reproducibility"
    reproducibility_files = (
        f"{reproducibility_heading}\n\n"
        f"{analysis_item}\n"
        f"{requirements_item}"
    )
    if reproducibility_heading not in markdown:
        raise ValueError("Quantitative README has no Reproducibility section")
    return markdown.replace(
        reproducibility_heading, reproducibility_files, 1
    )


def prepare_qualitative_markdown(markdown: str) -> str:
    """Remove package-level framing that is redundant on the web results page."""
    introduction = (
        "# Qualitative Supplementary Package\n\n"
        "This reviewer-facing package documents the qualitative component of a "
        "mixed-methods VR locomotion study. It combines the qualitative methods, "
        "analytical procedure, robustness checks, limitations, and approved "
        "qualitative results in a single document.\n\n"
    )
    if introduction not in markdown:
        raise ValueError("Expected qualitative package introduction not found")
    return markdown.replace(introduction, "", 1)


def counterbalancing_table(source: Path) -> str:
    path = source / "counterbalancing.csv"
    with path.open(encoding="utf-8", newline="") as stream:
        rows = list(csv.DictReader(stream))
    headers = {
        "Pattern": "Pattern",
        "MethodOrder": "Method order",
        "Block1Routes": "Block 1 routes",
        "Block2Routes": "Block 2 routes",
        "Block3Routes": "Block 3 routes",
        "Participants": "Participants",
    }
    head = "".join(f"<th>{label}</th>" for label in headers.values())
    body = "".join(
        "<tr>" + "".join(
            f"<td>{html.escape(row[column]).replace(' &gt; ', ' → ')}</td>"
            for column in headers
        ) + "</tr>"
        for row in rows
    )
    download = (
        '<div class="context-file-actions"><a class="file-link" '
        'href="materials/quantitative/counterbalancing.csv" download>Download</a></div>'
    )
    return (
        '<h3 id="counterbalancing">Counterbalancing</h3>'
        '<p>The table reports the method order and within-block route rotation '
        'used for every participant in the final sample. Block route columns '
        'correspond to the methods shown in the method-order column.</p>'
        f'{download}<div class="table-scroll"><table class="data-table">'
...
def build() -> None:
    ...
    for kind, source in sources.items():
        ...
        if kind == "quantitative":
            markdown = prepare_quantitative_markdown(markdown)
        if kind == "qualitative":
            markdown = prepare_qualitative_markdown(markdown)
            markdown = markdown.replace(
